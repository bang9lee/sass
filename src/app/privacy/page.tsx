...
import { buildPageMetadata, getPrivacyCopy, resolveSupportedLang } from "@/lib/site-content";
...
export async function generateMetadata({ searchParams }: Props): Promise<Metadata> {
    ...
    return buildPageMetadata(currentLang, getPrivacyCopy(currentLang));
}

export default async function PrivacyPage({ searchParams }: Props) {
    const { lang } = await searchParams;
    const currentLang = resolveSupportedLang(lang);
    const copy = getPrivacyCopy(currentLang);

    return (
        <StaticContentShell lang={currentLang} title={copy.heading} intro={copy.intro}>
            <p className="-mt-4 text-sm leading-7 text-white/65">{copy.updatedLabel}</p>
